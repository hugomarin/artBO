...
switch($form):
	...
	case 'addNCResource':
		...
		eval('$tags = ' . $objectVars['resource_object'] . 'Helper::retrieveTags(' . $typeId . ');');
		$extra					= ($typeId != NULL) ? " AND resource_type_id = " . escape($typeId) : '';		
		eval('$objectResourcesResult = ' . $objectVars['object'] . 'Helper::select' . $objectVars['object'] . 's("AND ' . $objectVars['field_prefix'] . '_id = ' . $objectId . ' AND ' . $objectVars['field_prefix'] . '_resource_field_name = \'' . $fieldName . '\'");');
		?>
		  <h3>ASOCIAR ARCHIVOS</h3>
		  <div class="medio_selector_search">
			<form>
		...
		<input name="" type="submit" value="Buscar" />    
		  ...
		  <form>
			<div class="medio_selector" id="layerResourceList">
			  ...
			  foreach($tags as $tag)
			  {
			  	eval('$resourceResult = ' . $objectVars['resource_object'] . 'Helper::select' . $objectVars['resource_object'] . 's("' . $extra . ' AND resource_tags LIKE \'%' . $tag . '%\'");');
			  ?>
				<li><a href="javascript:void(0);" onclick="SimpleAJAXCall('index.php?form_popup.control/listNCResources/<?=urlencode($tag)?>/<?=$_GET[1]?>/<?=$_GET[2]?>/<?=$_GET[3]?>/<?=$_GET[4]?>/<?=$_GET[5]?>', ElementStateChanged, 'GET', 'window2');"><img src="imgcontrol/iconos_sistema/dir.gif" /><span><strong><?=$tag?></strong> (<?=$resourceResult["num_rows"]?> archivos)</span></a></li>
              <?php
			  }
			  ?>
			  </ul>
			</div>
			<a href="javascript:void(0);" onclick="SimpleAJAXCall('index.php?form_popup.control/uploadNCResource/<?=$_GET[1]?>/<?=$_GET[2]?>/<?=$_GET[3]?>/<?=$_GET[4]?>/<?=$_GET[5]?>', loadFormLayer, 'GET', 'window2');" class="agregar_a_medio">[ + CARGAR ARCHIVO ]</a>
			<p><?=$objectResourcesResult["num_rows"]?> archivo(s) agregados</p>
		  </form>	
		<?php
    break;	
	case 'listResources':
    ...
    <input name="" type="submit" value="Buscar" />    
      ...
      <form>
		...
		<?php
    break;
	case 'listNCResources':
		$tag 	   	= urldecode($_GET[1]);
		$objectId 	= $_GET[2];
		$single	  	= ($_GET[3] != '') ? $_GET[3] : NULL;
		$typeId	   	= $_GET[4];
		$fieldName 	= $_GET[5];
		$objectVars = unserialize(urldecode($_GET[6]));
		$extra		= ($typeId != NULL) ? " AND resource_type_id = " . escape($typeId) : '';
		eval('$objectResourcesResult = ' . $objectVars['object'] . 'Helper::select' . $objectVars['object'] . 's("AND ' . $objectVars['field_prefix'] . '_id = ' . escape($objectId) . ' 
																	  			 AND ' . $objectVars['field_prefix'] . '_resource_field_name = \'' . escape($fieldName) . '\'");');
		eval('$resources = ' . $objectVars['resource_object'] . 'Helper::retrieve' . $objectVars['resource_object'] . 's("AND resource_tags LIKE \'%' . escape($tag) . '%\'' . $extra . ' ORDER BY resource_name");');
		?>
      <h3>ASOCIAR ARCHIVOS</h3>
      <div class="medio_selector_search">
        <form>
    ...
    <input name="" type="submit" value="Buscar" />    
      ...
      <form>
        <div class="medio_selector">
		  ...
		  foreach($resources as $resource)
		  {
		  	eval('$objectResourceResult = ' . $objectVars['object'] . 'Helper::select' . $objectVars['object'] . 's("AND ' . $objectVars['field_prefix'] . '_id = ' . escape($objectId) . ' 
																	  			 	AND ' . $objectVars['field_prefix'] . '_resource_field_name = \'' . escape($fieldName) . '\'
																					AND resource_id = ' . $resource->__get('resource_id') . '");');
		  	if($objectResourceResult["num_rows"] > 0)
			{
				$onclick = "SimpleAJAXCall('index.php?control_resource.controller/deleteNCResourceRelation/" . $resource->__get('resource_id') . "/" . $objectId . "/" . $fieldName . "/" . $_GET[3] . "/" . $_GET[6] . "', ElementStateChanged, 'GET', 'resource" . $resource->__get('resource_id') . "'); SimpleAJAXCall('index.php?control_resource.controller/refreshNCResourceList/" . $objectId . "/" . $fieldName . "/" . $_GET[3] . "/" . $typeId . "/" . $_GET[6] . "', ElementStateChanged, 'GET', '" . $fieldName . "'); ";
				$message = "[ quitar ]";
			}
			else
			{
				$onclick = "SimpleAJAXCall('index.php?control_resource.controller/addNCResourceRelation/" . $resource->__get('resource_id') . "/" . $objectId . "/" . $fieldName . "/" . $_GET[3] . "/" . $_GET[6] . "', ElementStateChanged, 'GET', 'resource" . $resource->__get('resource_id') . "'); SimpleAJAXCall('index.php?control_resource.controller/refreshNCResourceList/" . $objectId . "/" . $fieldName . "/" . $_GET[3] . "/" . $typeId . "/" . $_GET[6] . "', ElementStateChanged, 'GET', '" . $fieldName . "'); ";
				$message = "[ agregar ]";
			}
			$resourceType = new ResourceType($resource->__get('resource_type_id'));
			if($resourceType->__get('resource_type_name') == 'Image')
        ...
        <a href="javascript:void(0);" onclick="SimpleAJAXCall('index.php?form_popup.control/uploadNCResource/<?=$_GET[2]?>/<?=$_GET[3]?>/<?=$_GET[4]?>/<?=$_GET[5]?>/<?=$_GET[6]?>', loadFormLayer, 'GET', 'window2');" class="agregar_a_medio">[ + CARGAR ARCHIVO ]</a>
        <p><?=$objectResourcesResult["num_rows"]?> archivo(s) agregados</p>
      </form>
		<?php
    break;	
	...
	?>
	  ...
	  <form enctype="multipart/form-data" id="validable" method="post" target="imageFrame" action="index.php?control_resource.controller" name="resourceForm" onsubmit="document.getElementById('loadingImage').style.display = '';">
        ...
        <input type="hidden" name="fieldName" value="<?=$fieldName?>" />
        <input type="hidden" name="key" value="<?=$key?>" />
		<input type="hidden" name="object_vars" value="<?php echo $_GET[5]?>" />
        <input type="hidden" name="action" value="uploadNCResource" />
		<div class="medio_list" id="layerResourceList">
		  <ul>
			<li style="display:none" id="loadingImage"><img src="imgcontrol/loading.gif" /><span>Cargando...</span></li>
		  </ul>
        ...
        <iframe name="imageFrame" estyle="display:none"></iframe>
        <div class="botones2"> <a href="javascript:void(0);" onClick="SimpleAJAXCall('index.php?control_resource.controller/acceptNCResources/<?=$key?>/<?=$_GET[5]?>', doNothing, 'GET', ''); SimpleAJAXCall('index.php?control_resource.controller/addNCResourcesRelation/<?=$key?>/<?=$objectId?>/<?=$fieldName?>/<?=$_GET[5]?>', doNothing, 'GET', ''); SimpleAJAXCall('index.php?control_resource.controller/refreshNCResourceList/<?=$objectId?>/<?=$fieldName?>/<?=$_GET[2]?>/<?=$typeId?>/<?=$_GET[5]?>', ElementStateChanged, 'GET', '<?=$fieldName?>'); closeFormLayer('window2');"><img src="imgcontrol/bot_l_guardar.gif" border="0" /></a> <a href="javascript:void(0);" onClick="SimpleAJAXCall('index.php?control_resource.controller/notAcceptNCResources/<?=$key?>/<?=$_GET[5]?>', doNothing, 'GET', ''); closeFormLayer('window2');"><img src="imgcontrol/bot_l_cancelar.gif" border="0" /></a> </div>      
      
		<a href="javascript:void(0);" onclick="SimpleAJAXCall('index.php?form_popup.control/addNCResource/<?=$_GET[1]?>/<?=$_GET[2]?>/<?=$_GET[3]?>/<?=$_GET[4]?>/<?=$_GET[5]?>', ElementStateChanged, 'GET', 'window2');" class="agregar_a_medio">[ o SELECCIONAR UN ARCHIVO]</a>
	  </form>
	<?php
    break;	
	case 'updateResource':
	...
	case 'updateNCResource':
		...
		eval('$objectResource = new ' . $objectVars['object'] . '(' . $_GET[1] . ');');
		eval('$resource 	  = new ' . $objectVars['resource_object'] . '(' . $objectResource->__get('resource_id') . ');');
		$fieldName		 = $_GET[2];
		$objectId		 = $_GET[3];
		?>
          <h3>EDITAR RECURSO</h3>
          <form enctype="multipart/form-data" id="validable1" method="post" target="imageFrame" action="javascript:void(0);">
            <input type="hidden" name="action" value="updateNCResource"  />
            <input type="hidden" name="resource_id" value="<?=$resource->__get('resource_id')?>" />
			<input type="hidden" name="object_vars" value="<?php echo $_GET[4]?>" />
            <div class="panel_layer">
              <label> <span>Nombre</span>
              <input type="text" name="resource_name" title="Nombre" value="<?=$resource->__get('resource_name')?>" />
              </label>
            ...
            <iframe name="imageFrame" style="display:none"></iframe>
            <div class="botones2"> <a href="javascript:void(0);" onClick="FormParseAJAXCall('index.php?control_resource.controller', doNothing, 'POST', '', document.getElementById('validable1')); SimpleAJAXCall('index.php?control_resource.controller/refreshNCResourceList/<?=$objectId?>/<?=$fieldName?>/1/<?=$resource->__get('resource_type_id')?>/<?php echo $_GET[4]?>', ElementStateChanged, 'GET', '<?=$fieldName?>'); closeFormLayer('window2');"><img src="imgcontrol/bot_l_guardar.gif" border="0" /></a> <a href="javascript:void(0);" onClick="closeFormLayer('window2');"><img src="imgcontrol/bot_l_cancelar.gif" border="0" /></a> </div>      
          </form>
        <?php
	break;		
	case 'setLinkModule':
...
<a href="javascript:void(0);" onclick="<?=$backOnclick?>" class="agregar_a_medio">[ REGRESAR ]</a>            
	...
	break;	
	case 'rangeForm':
		$product 		= new Product(escape($_GET[1]));
		$range  		= isset($_GET[2]) ? new ProductRange($_GET[2]) : new ProductRange();
		$action			= isset($_GET[2]) ? 'update' : 'create';
		?>
          <h3>CREAR CAMPO</h3>
          <form id="validable1" method="post" action="index.php?control_shop_product_range.controller">
          	<input type="hidden" name="action" value="<?php echo $action?>" />
            <input type="hidden" name="product_id" value="<?=$product->__get('product_id')?>" />
			<input type="hidden" name="product_range_id" value="<?=$range->__get('product_range_id')?>" />
            <div class="panel_layer">
              <label> <span>Rango</span>
              <input type="text" name="product_range_number" title="Label" value="<?php echo $range->__get('product_range_number')?>"  />
              </label>
              <label> <span>Precio de lista</span>
              <input type="text" name="product_range_default_value" title="Label" value="<?php echo $range->__get('product_range_default_value')?>"  />
              </label>   
              <label> <span>Transporte interno USA</span>
              <input type="text" name="product_range_transport" title="Label" value="<?php echo $range->__get('product_range_transport')?>"  />
              </label>   	
              <label> <span>Margen (%)</span>
              <input type="text" name="product_range_margin" title="Label" value="<?php echo $range->__get('product_range_margin')?>"  />
              </label>   			              
            </div>
            <div class="botones2"><input type="image" src="imgcontrol/bot_l_guardar.gif" border="0" /> <a href="javascript:void(0);" onClick="closeFormLayer('window2');"><img src="imgcontrol/bot_l_cancelar.gif" border="0" /></a> </div>
        <?php	
	break;	
	case 'marcationRangeForm':
		$marcation 		= new Marcation(escape($_GET[1]));
		$range  		= isset($_GET[2]) ? new MarcationRange($_GET[2]) : new MarcationRange();
		$action			= isset($_GET[2]) ? 'update' : 'create';
		?>
          <h3>CREAR CAMPO</h3>
          <form id="validable1" method="post" action="index.php?control_shop_marcation_range.controller">
          	<input type="hidden" name="action" value="<?php echo $action?>" />
            <input type="hidden" name="marcation_id" value="<?=$marcation->__get('marcation_id')?>" />
			<input type="hidden" name="marcation_range_id" value="<?=$range->__get('marcation_range_id')?>" />
            <div class="panel_layer">
              <label> <span>Rango</span>
              <input type="text" name="marcation_range_number" title="Label" value="<?php echo $range->__get('marcation_range_number')?>"  />
              </label>
              <label> <span>Valor unitario</span>
              <input type="text" name="marcation_range_value" title="Label" value="<?php echo $range->__get('marcation_range_value')?>"  />
              </label>   
            </div>
            <div class="botones2"><input type="image" src="imgcontrol/bot_l_guardar.gif" border="0" /> <a href="javascript:void(0);" onClick="closeFormLayer('window2');"><img src="imgcontrol/bot_l_cancelar.gif" border="0" /></a> </div>
        <?php	
	...
	$objectVars = isset($objectVars) ? $objectVars : unserialize(urldecode($_GET[2]));
	$objectResourceId =  isset($_GET[1]) ? $_GET[1] : $_POST['resource_id'];
	eval('$objectResource = new ' . $objectVars['object'] . '(' . $objectResourceId . ');');
	$resourceId = $objectResource->__get('resource_id');	
	$thumb = isset($_GET[3]) ? urldecode($_GET[3]) : NULL;
	...
	if($thumb != NULL)
	{
	  ...
	  <h3>ALISTAMIENTO DE IMAGENES</h3>
		<form name="form" action="javascript:void(0);" method="post" id="resourceForm">
			...
			<input id="newSize" name="newSize" value="<?=$size[0]?>,<?=$size[1]?>" readonly="readonly" type="hidden" />
			<input type="hidden" name="crop" value="1" />
            <input type="hidden" name="form" value="thumbFormNC" />
			<input type="hidden" name="resource_id" value="<?=$objectResourceId?>" />
			<input type="hidden" name="object_vars" value="<?php echo urlencode(serialize($objectVars));?>" />
			<p>Ajuste las imagenes para la presentaci&oacute;n en el sitio web</p>
			<div class="botones2">
			  <input name="" type="image" onclick="FormParseAJAXCall('index.php?form_popup.control', ElementStateChanged, 'POST', 'window2', document.getElementById('resourceForm'));" src="imgcontrol/bot_l_guardar.gif" alt="Guardar" width="160" height="28" />
			</div>
	...
	{
		$_GET[2] = isset($_GET[2]) ? $_GET[2] : urlencode($_POST['object_vars']);
		$thumbs     = ResourceHelper::retrieveThumbFolders();
		echo '<div class="medio_selector"><ul>';
        foreach($thumbs as $thumb)
        {
            if($thumb['thumb_type'] != 'resize')
            {
                echo '<li><a href="javascript:void(0)" onclick="ParamsAJAXCall(\'index.php?form_popup.control/thumbFormNC/' . $objectResourceId . '/' . $_GET[2] . '/' . urlencode($thumb) . '\', createCropperPopup, \'GET\', \'window2\', \'' . $thumb . '\')"><img src="resources/images/' . $thumb . '/' . $resource->__get('resource_file') . '?' . rand(0,9999) . '" width="100" /><span>' . $resource->__get('resource_name') . ' - ' . $thumb . '</span></a></li>';	
            }
        }
        echo '</ul></div>    <p>Ajuste las imagenes para la presentaci&oacute;n en el sitio web</p>
				<div class="botones2">
	...
	case 'listRelationContents':
		...
		$contents_relationship = contentHelper::retrieveContents(' AND module_id =' . $module_id . $filter);
	...
	case 'resourceOrderPopupNC':
		...
		eval('$objectResources = ' . $objectVars["object"] . 'Helper::retrieve' . $objectVars["object"] . 's("AND ' . $objectVars["field_prefix"] . '_id = ' . escape($objectId) . ' 
																	  AND ' . $objectVars["field_prefix"] . '_resource_field_name = \'' . escape($fieldName) . '\' ORDER BY ' . $objectVars["field_prefix"] . '_resource_order");');
        ...
        foreach($objectResources as $objectResource)
		{
			...
			if($resourceType->__get('resource_type_name') == 'Image')
				...
				$imgSrc 	 = 'imgcontrol/iconos_sistema/' . $resourceType->__get('resource_type_icon');
			?>
		  <li id="item_<?=$objectResource->__get($objectVars["field_prefix"] . '_resource_id')?>"> <a href="javascript:void(0);"><img src="<?=$imgSrc?>" />
			<?=$resource->__get('resource_name');?>
			</a> </li>
			<?php
		}				  
		...
		<form id="serializeForm" method="post" action="index.php?control_resource.controller">
			<input type="hidden" name="serializedArray" value="" />
			<input type="hidden" name="action" value="setOrderNC" />
			<input type="hidden" name="contentId" value="<?=$_GET[1]?>" />
			<input type="hidden" name="fieldName" value="<?=$_GET[2]?>" />
			<input type="hidden" name="object_vars" value="<?=$_GET[3]?>" />
			<div class="botones2"> <a href="javascript:void(0);" onClick="document.getElementById('serializeForm').submit();"><img src="imgcontrol/bot_l_guardar.gif" border="0" /></a> </div>
		</form>
	<?php
	break;	
	case 'setCoords':
		...
		$parent			= new Content($content->__get('content_parent'));
		$imageFirsth 	= ResourceHelper::getGallery($parent->__get('content_id'),'content_extra_varchar_1');
		if (count($imageFirsth) > 0)
			$imageFirsth= $imageFirsth[0]->__get('resource_file');
		else
			$imageFirsth='';
		?>
          ...
          <form  enctype="multipart/form-data" name="contentForm" id="contentForm" >
            <input type="hidden" id="coords" name="content_varchar_2" value="" />
            <div class="panel_layer">
              <label> 
              
			<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=9,0,28,0" width="557" height="307">>
			    <param name="movie" value="swf/setCoords.swf" />
                <param name="quality" value="high" />
                <param name="FlashVars" value="map=<?php echo $imageFirsth?>" />
                <embed src="swf/setCoords.swf" width="557" height="307" quality="high" pluginspage="http://www.adobe.com/shockwave/download/download.cgi?P1_Prod_Version=ShockwaveFlash" type="application/x-shockwave-flash" FlashVars="map=<?php echo $imageFirsth?>"></embed>
              </object>
</label>
            </div>
            <div class="botones2">
            <input type="hidden" name="fieldName" value="content_varchar_2" />
...
?>
		  ...
		  <div class="botones2"> <a href="javascript:void(0);" onClick="closeFormLayer('window2');"><img src="imgcontrol/bot_l_guardar.gif" border="0" /></a> <a href="javascript:void(0);" onClick="closeFormLayer('window2');"><img src="imgcontrol/bot_l_cancelar.gif" border="0" /></a> </div>
         <?php
	break;
	case 'updateTrees':
		?>
          <h3>CARGAR ARBOLES</h3>

          <form enctype="multipart/form-data" id="validable1" method="post" action="index.php?control_specie.controller">

          	<input type="hidden" name="action" value="updateTree" />

            <div class="panel_layer">

              <label> <span>Archivo XLS</span>

              <input type="file" name="admitteds" title="Archivo" />
